...
function events() {
    for (let i = 16; i < 24; i++) {
        ...
        cards[i].addEventListener('click', function () {
            ...
            if (i < 20) {
                for (let j = 16; j < i + 1; j++) { // i+1 to disable card stacking
                    if (cards[j].style.top !== '208px') {
                        cards[j].style.left = parseInt(cards[j].style.left) + 40 + 'px';
                    }
                }
            } else {
                for (let j = (i + 1); j < 24; j++) { // i+1 to disable card stacking
                    if (cards[j].style.top !== '208px') {
                        cards[j].style.left = parseInt(cards[j].style.left) - 40 + 'px';
                    }
                }
...
let arr = [0, 1, 2, 3, 4, 5, 6, 7]; // array to disable card repeating
...
function oppMove() {
    ...
    let c = arr[x] + 24; // random op-card 24-31
    ...
    arr.splice(x, 1); // deleting arr items to lock card repeating

    if (c < 28) {
        for (let j = 24; j < c + 1; j++) { // i+1 to disable card stacking
            if (cards[j].style.top !== '208px') {
                cards[j].style.left = parseInt(cards[j].style.left) + 40 + 'px';
            }
        }
    } else {
        for (let j = c; j < 32; j++) { // i+1 to disable card stacking
            if (cards[j].style.top !== '208px') {
                cards[j].style.left = parseInt(cards[j].style.left) - 40 + 'px';
            }
        }
